...
@router.oauth2.get('/accounts/all/entries',
                   response_model=list[EntrySelectAllResult])
...
@router.oauth2.get('/accounts/{discord_id}/entries',
                   response_model=list[EntrySelectAllResult])
async def get_account_entries(discord_id: int,
                              type: ENTRY_SELECT_ALL_MEDIA_TYPE | None = None):
    resp = await entry_select_all(get_edgedb(),
                                  media_type=type,
                                  discord_id=discord_id)
    if resp is None:
        resp = []
...
async def get_medias(ids_al: str):
    try:
        ids_al_parsed = ([int(id_al) for id_al in ids_al.split(',')]
                         if len(ids_al) > 0 else [])
    except ValueError:
...
async def media_search(type: MEDIA_TYPES, search: str):
    async with get_meilisearch() as client:
        ...
        resp = await index.search(
            search,
            limit=25,
            filter=f"type={type}" if type is not None else None)
...
@router.oauth2.get('/medias/autocomplete',
                   response_model=list[MediaTitleAutocompleteResult])
async def media_title_autocomplete(search: str,
...
@router.oauth2.get('/medias/{id_al}/entries',
                   response_model=list[EntrySelectFilterMediaResult])
async def get_media_list_entries(id_al: int):
    return await entry_select_filter_media(get_edgedb(), id_al=id_al)


@router.oauth2.get('/medias/{id_al}/edges/charas',
                   response_model=list[CEdgeSelectFilterMediaResult])
...
@router.oauth2.get('/charas/autocomplete',
                   response_model=list[CharaNameAutocompleteResult])
...
@router.public.get(
    ...
    response_class=StreamingResponse,
    responses={status.HTTP_400_BAD_REQUEST: dict(model=HTTPExceptionModel)})
async def get_chara_collage(ids_al: str,
                            hide_no_images: int = 0,
                            blooded: int = 0):
    try:
        ids_al_parsed = ([int(id_al) for id_al in ids_al.split(',')]
                         if len(ids_al) > 0 else [])
    ...
    if len(ids_al_parsed) == 0:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST)
    return StreamingResponse(chara_collage(ids_al_parsed,
                                           hide_no_images=bool(hide_no_images),
...
@router.oauth2.get('/staffs/autocomplete',
                   response_model=list[StaffNameAutocompleteResult])
